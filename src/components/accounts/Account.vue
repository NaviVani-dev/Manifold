<template>
  <div class="w-76 h-50 bg-base-200 rounded-md flex flex-col gap-2 relative">
    ...
    <div v-if="accounts.fetchStatus === 'done'" class="w-full h-full absolute inset-0 overflow-hidden rounded-md">
      <img
        v-if="acc.steam[0]?.face || acc.face"
        :src="acc.steam[0]?.face || acc.face"
      ...
      <video
        autoplay muted loop
        v-if="acc.steam[0]?.anim_bg"
        :src="acc.steam[0]?.anim_bg"
        class="w-full h-full object-cover absolute inset-0"
      />
    </div>
    ...
    <div v-if="accounts.fetchStatus === 'done'" class="absolute left-4 top-4 w-26 h-26 z-[1] border-b-2 border-accent">
      <div v-if="!acc.steam[0]?.anim_face" class="w-full h-full bg-base-300 flex items-center justify-center border-b-2 border-accent">
        <UserRound :size="62"/>
      </div>
      <img
        v-if="acc.steam[0]?.anim_face || acc.steam[0]?.face || acc.face"
        :src="acc.steam[0]?.anim_face || acc.steam[0]?.face || acc.face"
        class="w-full h-full absolute inset-0"
      />
      <img
        v-if="acc.steam[0]?.frame"
        :src="acc.steam[0]?.frame"
    ...
    <div class="absolute bottom-0 w-full h-1/2 p-4 pt-10 flex flex-col justify-start bg-gradient-to-b from-[rgba(24,26,30,0.85)] from-[5%] to-[rgba(24,26,30,0.65)] to-[95%]">
      <div class="flex flex-row items-center gap-1">
        ...
        <p class="text-xl">{{ acc.username }}</p>
      </div>
      <div class="flex flex-row items-center gap-1">
        <Steam :size="12" />
        <p v-if="acc.steam[0]?.username" class="text-sm">{{ acc.steam[0]?.username }}</p>
        <div v-if="acc.steam.length > 1" class="tooltip" :data-tip="$t('accounts.multiple_accs')">
          <p class="text-xs text-base-content/50 mt-0.5">+{{ acc.steam.length - 1 }}</p>
        </div>
        <div v-if="!acc.steam[0]?.username" class="tooltip" :data-tip="$t('accounts.no_steam_acc_tip')">
          <p class="text-sm">{{ $t('accounts.no_steam_acc') }}</p>
        </div>
      </div>
    </div>
    <AccountOptions :accountIndex="d.accountIndex" />
  </div>
</template>
...
// if this is rendering its bc accounts already exists, it should be impossible to render it without it
